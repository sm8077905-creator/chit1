Добавлена функция **полета (Fly)** с настраиваемой скоростью (`FlySpeed`), а также сохранены все гибкие настройки (ползунки и выпадающие списки) для каждого из модулей чиста.

Управление полетом полностью адаптировано под мобильные устройства (Delta): персонаж летит туда, куда смотрит камера, а для набора высоты достаточно использовать кнопку прыжка.

### Финальный Rage-скрипт с полетом, ползунками и полным функционалом:
